...
source(file.path("functions", "emote_number.R"))

plot_emote_number <- function(table1, who1, table2, who2, table3, who3){
  data_to_plot <- rbind(emote_number(table1, who1), emote_number(table2, who2), emote_number(table3, who3))
  ...
  plot <- data_to_plot %>%
    ...
    ggplot(aes(x = Messenger, y = number_of_emotes, fill = Messenger)) + 
    ...
    theme_minimal() +
    labs(x="", y = "Number of received reactions") +
    scale_y_continuous(expand = c(0, 0)) +
    theme(
