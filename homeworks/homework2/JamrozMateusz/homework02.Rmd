...
# Filtracja kodów pocztowych z ponad 280 nieruchomościami
zip_counts <- df %>%
  group_by(zipcode) %>%
  summarise(n=n()) %>%
  filter(n > 280)

# Wybranie głównych danych dla wyselekcjonowanych kodów pocztowych
df_filtered <- df %>%
  filter(zipcode %in% zip_counts$zipcode)

# Obliczenie mediany oraz odchylenia standardowego ceny dla każdego roku i kodu pocztowego
...
# Tworzenie wykresu rozkładu ceny w zależności od powierzchni mieszkalnej, uwzględniając obecność piwnicy
...
# Wykres słupkowy dla procentowego udziału wysokiej jakości w zależności od liczby pięter
