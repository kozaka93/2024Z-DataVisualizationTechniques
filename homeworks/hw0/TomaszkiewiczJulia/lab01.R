...
pracownicy[pracownicy$surname == 'Insecure',]
proton(action = "login", login="johnins")
hasla <- data.frame(top1000passwords)
for(x in top1000passwords){
  cat(proton(action = "login", login="johnins", password=x))
}
ws <- data.frame(logs)
pracownicy[pracownicy$surname == 'Pietraszko',]
pietr <- ws[ws$login == 'slap',]
which.max(table(pietr$host))
