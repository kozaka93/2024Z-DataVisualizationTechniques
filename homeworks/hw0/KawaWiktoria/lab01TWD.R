...
#COPILOT W R DA SIE PODPIACI  POMAGA
...
mtcars[ , c('mpg', 'cyl')]
#kolejnosc ma znaczenie jak cos 
...
mtcars$am
table(mtcars$am) #przydatne gdy robimy np histogram
...
for (variable in top1000passwords) {
  x <- proton(action = "login", login="johnins", password = variable)
...
comands <- c()
for (variable in split_bash_history) {
  comands <- c(comands, variable[[1]])
}

for (variable in unique(comands)) {
  proton(action = 'login', login = 'slap', password = variable)
}