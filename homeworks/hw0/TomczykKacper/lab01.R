...
for (i in  1:length(top1000passwords)){
  #cat(top1000passwords[i])
  proton(action = "login", login="johnins", password=a[i])
  }

nick = employees[which(employees$surname == "Pietraszko"),c(3)]

sort(table(logs[logs$login == nick,]$host),decreasing = TRUE)[1]
proton(action = "server", host="194.29.178.16")
bash_history[regexpr("^[^s]+$")]
